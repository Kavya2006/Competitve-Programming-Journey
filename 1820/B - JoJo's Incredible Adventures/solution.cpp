...
// ------------------------- Main Logic ------------------------- //
/*
can we say suppose maximum ones is k suppose
so initial width=k , height=1
then width =(k-1) , height=2
thrn width =(k-2) , height=3
and so on
and after multiplying we take maximum
suppose a function , y=(k-x)(x+1)
where x takes value from 0 to k-1
y=kx-k-x*x-x
so diff , we get- 2*x-1+k=0
x=(k-1)/2
so we can take x=(k-1)/2 , or to do ceil divison , (k)/2
and put in function to get max 
 
*/
 
void Solve() {
    ...
    ll x1=(maxi-1)/2;
    ll x2=(maxi+1)/2;
    cout<<max((maxi-x1)*(x1+1),(maxi-x2)*(x2+1))<<endl;
 
