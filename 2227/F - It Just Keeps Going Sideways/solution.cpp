...
sort(b.begin(),b.end()); // final array will be the sorted array
// basically for which indices we need to find how many people crossed it from left to right
ll count=0;
ll suma=0;
ll sumb=0;
for(ll i=0 ; i<n ; i++){
    suma+=a[i];
    sumb+=b[i];
    // so crossed first point will be initial - final = suma-sumb
    count+=(suma-sumb);
}
// now we are removing one of the blocks and need to find extra movement
// so basically if in sorted array a block was at position 2 and in normal its at position 5
// if we remove one block from it , count will gain 3 extra blocks , because suma-sumb will 
// give extra 1 till we reach 3rd position
// so we need to find the max of (element position in sorted - element position normally)
// and obviously if a element is twice like we would erase its first occurence from b
// otherwise it wont be sorted
ll temp=0;
for(ll i=0 ;i<n ; i++){
