...
vector<pair<ll,pair<ll,ll>>>ans(n+m);
ll l=0;
ll r=0;
for(ll i=0 ; i<ans.size(); i++){
  if(l==n){
    ...
    ans[i].first=b[r];
    r++;
    ans[i].second.first=l;
    ans[i].second.second=r;
  }
  else if(r==m){
     ans[i].first=a[l];
    l++;
...
vector<ll>prefixans(n+m);
for(ll i=0 ; i<n+m ; i++){
    prefixans[i]=ans[i].first;
    if(i>0){
...
while(q--){
    ll x,y,z; cin>>x>>y>>z;
     ...
     if(ans[z].second.first>x){
        ll k=ans[z].second.first;
        
        anss-=(prefixa[k-1]);
        if(x>0){
            anss+=prefixa[x-1];
        }
        if(ans[z].second.second>0){
        anss -= prefixb[ll(ans[z].second.second)-1LL];
        }
        anss+=prefixb[z-x];
 
     }
     else if(ans[z].second.second>y){
         ll k=ans[z].second.second;
        
        anss-=(prefixb[k-1]);
        if(y>0){
            anss+=prefixb[y-1];
        }
        if(ans[z].second.first>0){
        anss -= prefixa[ll(ans[z].second.first)-1LL];
        }
        anss+=prefixa[z-y];
