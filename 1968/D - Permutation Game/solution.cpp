...
 
 
vector<ll>prefix1(bodya.size());
vector<ll>prefix2(shasha.size());
prefix1[0]=bodya[0];
prefix2[0]=shasha[0];
for(ll i=1 ; i<bodya.size();  i++){
    prefix1[i]=prefix1[i-1]+bodya[i];
}
for(ll i=1 ; i<shasha.size(); i++){
    prefix2[i]=prefix2[i-1]+shasha[i];
}
ll maxi1=k*bodya[0];
ll maxi2=k*shasha[0];
 
for(ll i=1 ; i<min(ll(bodya.size()),k);  i++){
    maxi1=max(maxi1,(k-i)*bodya[i]+prefix1[i-1]);
}
for(ll i=1 ; i<min(ll(shasha.size()),k);  i++){
    maxi2=max(maxi2,(k-i)*shasha[i]+prefix2[i-1]);
}
if(maxi1>maxi2){
    cout<<"Bodya
";
