...
// ------------------------- Main Logic ------------------------- //
/*
so suppose i have l1 elements with freq f1
l2 elements with freq f2
l3 element with freq f3 and so on
 
and have l4 elements whoch are equal to c
so like if l4 ==0 we can just say ans is max freq out of all the freq
 
but if l4!=0 , there could be some combination to take some l1 elements in range
or maybe some l2
and its not necesaary like only highest freq element we can select
like suppose its
c=2
1212121333 , here 1 has highedt freq
but best answer would be to take 3 '3 so 3+3=6 willl be answer
 
so i should store them in set something maybe
 
so if i can just loop maybe 
 
so kadane type thing to be used
so can i just say that , answer= no of something + no of c outside this interval
=no of something + total c- no of c inside
so total=constant basically ik that
so  ans->    total+ inside that interval (something - no of c)
 
so suppose i havw this only
c=2
1212121333 , tot c=3
so i go at 1 , here i check 1=1 , inside =0 , so max ans =4 till now
now i go to 2, so c inside --; , 
now i go at 1 again , now from this 1 to last 1 , c has dec by 1
so there is no benefit in carrying the last 1 with me now coz ans would get shorter or equal only , so i drop that 1 , and see with this only , cavail=3
1=1 , so ans=4 
now i go ahead another c, decrease inside c, inside--
now i go at 1, same not good to take that move on
now i got at 2 minus minus and move on......
so now when i come at 3 , outside=3 , so ans=4 here also
when i move at next 3 , between these two three's prefix of c doesnt change
so i can keep last position as initial 3 only and ans=2+3=5
now next 3 , between this and first 3 ans not change so ans=max(ans,3+3)
so ans=6 , so yeah ig i can do this now
*/
ll n,c; 
vector<ll>a;
 
void Solve() {
  ...
  vector<ll>lastseen(1e6,-1);
  vector<ll>prefix(n+1,0);
  map<ll,ll>freq;
  for(ll i=1; i<=n; i++){
    prefix[i]=prefix[i-1]+(a[i-1]==c);
  }
  ll ans=tot;
  for(ll i=1; i<=n; i++){
     if(a[i]==c){
     ...
     if(lastseen[a[i]]==-1){
        ...
        ll diff=prefix[i]-prefix[lastseen[a[i]]];
        if(diff<freq[a[i]]){
            freq[a[i]]++;
            ans=max(ans,freq[a[i]]+tot-diff);
        }
        else{
            lastseen[a[i]]=i;
            freq[a[i]]=1;
