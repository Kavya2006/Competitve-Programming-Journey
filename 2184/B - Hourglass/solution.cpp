...
void Solve(){
ll s,k,m; cin>>s>>k>>m;
// fuck last chance lets frame carefully
/* if k>m then max(s-m,0)
 
*/
if(k>m){
...
/*
now what if k==m , suppose 3 10 10
at last second will turn around , so 3 seconds
but in 15 10 10 , 5 will be left but timer reset to 15-5 =10;
 
but in 4 11 11 ; 
*/
if(k==m){
    if(s>=k){
cout<<s-max(s-m,0LL)<<endl;
return;
}
...
now m>k
suppose 7 13 30
after 13 seconds flip and timer start 
then at 26 again flip and timer start
so if s<=k
m=30%13= 4
so situation becomes 7 13 4
here answer will be 3
 
buttt if i see 7 15 45 something like this
then last second again start
 
*/
if(m>k){
if(s<=k){
...
 
    ...
    // suppose 40 13 50
    // s oscillate between 27 and 40
    // if m/k is even s=40 at that time else 27;
    if((m/k) %2==0){
        if(m%k==0){
            cout<<s<<endl;
        }
