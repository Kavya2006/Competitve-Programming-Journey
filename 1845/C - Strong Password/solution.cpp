...
 
// ------------------------- Main Logic ------------------------- //
// so like i have to do some rec/dp shit , first like suppose we starting at 0
// if we reach m we so seq exist in string , like if we reach end of string look for seq
// we can say no seq exist and it is true
 
 
string s,l,r;
ll m; ll n;
...
 
    ...
    for(ll j=ll(l[i])-'0'; j<= ll(r[i])-'0'; j++){
        ll lastidx=-INF;
         for(ll k=last+1; k<n; k++){
            if(ll(s[k])-'0'==j){
                lastidx=k;
                break;
            }
...
void Solve() {
    ...
    cin>>l>>r;
    pos=0;
    vis.assign(m+5,vector<ll>(n+5,0));
    rec(0,-1);
    if(pos){
        cout<<"YES
";
