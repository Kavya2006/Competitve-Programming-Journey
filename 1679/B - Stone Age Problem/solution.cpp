...
void push(ll i, ll l, ll r) {
    ...
    if(l==r){
        return; // leaf dont have any children
    }
    ll mid = (l + r) / 2;
    ll x = Segtree[i].lazy;
 
    // 1. Pass the lazy tag to the Left Child
    Segtree[2 * i].lazy = x;
    Segtree[2 * i].isLazy = true; // FIX 3
   // Segtree[2 * i].mini = x;
   // Segtree[2 * i].maxi = x;
    Segtree[2 * i].sum = (mid - l + 1) * x;
 
    // 2. Pass the lazy tag to the right child
    Segtree[2 * i + 1].lazy = x;
    Segtree[2 * i + 1].isLazy = true; 
   // Segtree[2 * i + 1].mini = x;
   // Segtree[2 * i + 1].maxi = x;
    Segtree[2 * i + 1].sum = x * (r - mid); 
 
    Segtree[i].isLazy = false;
}
 
 
void build(ll i , ll l, ll r){
...
 
void update(ll i, ll l, ll r, ll lq, ll rq, ll x) {
    if (lq <= l && rq >= r) {
        Segtree[i].sum = x * (r - l + 1);
       // Segtree[i].maxi = x;
      //  Segtree[i].mini = x;
        Segtree[i].isLazy = true;
        Segtree[i].lazy = x;
        return; 
    }
    push(i, l, r);
