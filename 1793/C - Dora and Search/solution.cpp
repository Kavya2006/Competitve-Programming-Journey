...
// ------------------------- Main Logic ------------------------- //
/*
so if length is 3 , then ans is always -1 , coz something will get true
// so we want a perm to go down then go up
or a perm can go up and then go down
and diff bet
if starting element is not 1 and ending is not n and vice versa
then ans is obviously yes
coz at least one element will come that will be bigger than both of them
and smaller than both of them
 
*/
 
void Solve() {
    ...
    // now either start is 1 or ending is n or like both things or vice versa
    // now suppose perm length is 7 suppose
    // so suppose if 1 comes at start and 7 comes somewhere
    // like 1746523
    // now here also we can take anything with 
    // so here 4 value and 3 value's index will be the answer like
    // so we need to let it go up and down
    ll l=0;
    ll r=n-1;
    ll mini=1;
    ll maxi=n;
