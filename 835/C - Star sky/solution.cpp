...
void Solve(){
// probably 2d prefix sum type shit
// what if i create 3d matrix and precompute for all c values already
ll n,q,c; cin>>n>>q>>c;
ll matrix[101][101][11];
for(ll i=0 ; i<=100 ; i++){
    for(ll j=0 ; j<=100 ; j++){
        for(ll k=0 ; k<=10 ;k++){
            matrix[i][j][k]=0;
...
for(ll i=0 ;i<n;i++){
    ll x,y,z; cin>>x>>y>>z;
    matrix[x][y][z]++;// basically counting all stars at position x,y with brightness z
}
// now i have to create 2d prefix sum
ll prefix[101][101][11];
for(ll i=0 ; i<=100 ; i++){
    for(ll j=0 ; j<=100 ; j++){
        for(ll k=0 ; k<=10 ;k++){
            prefix[i][j][k]=0;
...
for(ll i=1 ; i<=100; i++){
    for(ll j=1 ; j<=100 ; j++){
        for(ll k=0 ; k<=10 ; k++){
            prefix[i][j][k]=prefix[i-1][j][k]+prefix[i][j-1][k]+matrix[i][j][k]-prefix[i-1][j-1][k];
        }
    }
}
 
for(ll j=0 ; j<q; j++){
    ll ans=0;
    ll t1,x1,y1,x2,y2; cin>>t1>>x1>>y1>>x2>>y2;
    for(ll i=0 ;i<=c ; i++){
        ll count=prefix[x2][y2][i]-prefix[x1-1][y2][i]-prefix[x2][y1-1][i]+prefix[x1-1][y1-1][i];
        ans+=count*((i+t1)%(c+1));
    }
