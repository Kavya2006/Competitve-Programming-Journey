...
for(ll left=0 ; left<n-2 ; left++){
    for(ll right=left+2 ; right<n ; right++){
        ll middle=prefix[right-1]-prefix[left];
        if((prefix[left]%3!= suffix[right]%3 && prefix[left]%3!= middle%3 && middle%3!=suffix[right]%3) || (prefix[left]%3== suffix[right]%3 && prefix[left]%3== middle%3 && middle%3==suffix[right]%3)) {
