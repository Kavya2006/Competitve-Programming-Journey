...
// ------------------------- Main Logic ------------------------- //
/*
so from end we find max elements with same value and upto start
// then suppose n=11 , and last 2 elements have same and starting 1 also has same has them
// so remaining 8 elements , so ans=log2(8) is value ka ceil 
 
// but if suppose we have  1 2 4 5 8 5 ,  here if we give 8 the value 5
// so now we give 8 as 5 , so now we have  1 2 4 5 5 5 , now in 1 turn we can do
// so we should take at max o(nlogn)thingi
 
*/
 
void Solve() {
    ...
    while(b<n){
      ...
      ll lastnew=last-b;
      for(ll i=last-1; i>=max(0LL,lastnew); i--){
        a[i]=ok;
        b++;
      }
      ans++;
