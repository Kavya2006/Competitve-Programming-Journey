...
 
-3 4 2 -1 1 0
heere if i sort it -3 -1 0 1 2 4
so b1=a1
so smallest should be 1 used
so 1=1 , 1+0=1, 1+2=3, 3-1=2, 2+4=6, 6-3=3
so we divide into 2 parts and do b.s on both parts ig
and remove that from mutliset
 
*/
 
void Solve() {
    ...
    for(ll i=0; i<n; i++){
        ...
        // elem>=1-currsum
        if(!ms2.empty()){
        auto it=ms2.lower_bound(1-currsum);
        if(it!=ms2.end()){
            ll k=*it;
            ans[i]=(currsum+k);
            ms2.erase(it);
        ...
        if(!ok){
            if(!ms1.empty()){
        auto it2=ms1.lower_bound(1-currsum);
        if(it2!=ms1.end()){
            ll k2=*it2;
            ans[i]=(currsum+k2);
            ms1.erase(it2);
