...
";
    ...
    // so basically now we form difference array of size n-1
    // so like d[i]=a[i+1]-a[i]
    //         d[i-1]=a[i]-a[i-1]
    // now if we do a[i]=a[i+1]+a[i-1]-a[i]
    // d[i]=a[i-1]-a[i]
    // d[i-1]=a[i+1]-a[i]
    // so we see d[i] and d[i-1] getting swapped
    // so we can do this so many times and therefore form any perm of diff array
    // so if we sort both diff array of a and b and if they equal then possible
    // otherwise not
    // and we speratrely needed to check a[0]==b[0] && a[n-1]==b[n-1]
    // because these both can never be modified
    vector<ll>d1(n-1);
    vector<ll>d2(n-1);
    for(ll i=0; i<n-1; i++){
        d1[i]=a[i+1]-a[i];
