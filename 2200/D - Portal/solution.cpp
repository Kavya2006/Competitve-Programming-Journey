...
// see the elements between portal will stuck there and can rotate there only
// rest elements can only go left and right the remaining
vector<ll>stuck;
vector<ll>first;
vector<ll>last;
ll stuckmin=1e9;
...
// suppose 4 3 0 1 5 0 6 8
// stuck has smallest element so will push all in first to right side
// we get 1 5 4 3 6 8
vector<ll>newie;
for(ll i=0 ; i<first.size(); i++){
    newie.push_back(first[i]);
}
for(ll i=0 ; i<last.size(); i++){
    newie.push_back(last[i]);
}
first.clear();
last.clear();
bool possible=true;
for(ll i=0 ; i<newie.size(); i++){
   if(newie[i]<stuckmin && possible){
    first.push_back(newie[i]);
   }
   else{
    possible=false;
    last.push_back(newie[i]);
   } 
}
 
for(ll i=0 ; i<first.size(); i++){
    ...
    for(ll i=0 ; i<stuck.size(); i++){
        cout<<stuck[(i+stuckindex)%stuck.size()]<<" ";
 
 
  }
    for(ll i=0 ; i<last.size(); i++){
