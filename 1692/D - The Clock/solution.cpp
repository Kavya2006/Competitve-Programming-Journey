...
// ------------------------- Main Logic ------------------------- //
/*
so all palindromes are 00:00,01:10,02:20,03:30,04:40,05:50
...
void Solve() {
    string s; cin>>s;
    ll x; cin>>x;
    vector<ll>pali={0,70,140,210,280,350,601,671,741,811,881,951,1202,1272,1342,1412};
    ll start=((s[0]-'0')*10+(s[1]-'0'))*60+(s[3]-'0')*10+(s[4]-'0');
    set<ll>ss;
    set<ll>tt;
    for(ll i=0; i<pali.size(); i++){
        ss.insert(pali[i]);
    }
    ll ans=0;
    vector<bool>vis(1e4,0);
    for(ll i=0; i<=1e5; i++){
        if(vis[(start+i*x)%1440]){
            break;
        }
      auto it=ss.find((start+i*x)%1440);
      if(it!=ss.end()){
           ans++;
      }
      vis[(start+i*x)%1440]=1;
    }
    cout<<ans<<endl;
}
 
