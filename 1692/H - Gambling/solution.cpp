...
// ------------------------- Main Logic ------------------------- //
/*
4 4 3 4 4 , here length=2 and all=2 so 2 can be answwe
but if length=5 , good=4 , notgood=1 , so answer=8 dolllars so all is better
11 1 11 1 11
here any odd length would work like , if only single elemnt that also correct
 
*/
 
 
void Solve() {
    ...
    for(auto &j : pos){
        ...
        for(ll i=1; i<ok.size(); i++){
            ll dist=ok[i]-ok[i-1]-1;
            bestnow=bestnow-dist+1;
            if(bestnow<=0){
                bestnow=1;
                lbest=ok[i];
                rbest=ok[i];
