...
/*
in 3rd testcase
7 doesn't have child , so contri[7]=1;
6 also doesnt , so 1
now for 5 , h=0 is itself only , h=1 more than 1 present
so that will be taken into account
 
so thing is if someone has 2 children
suppose one go three places deep , another go 4
so h=4 would have already been covered so h=3 will be unique as going 2
so we would add 3 to the answer thing
 
so in this 3rd testcase , all individual contri=7
now 5 has max depth of 1 so +1 , and 1 has 2 depth of 1 so +1
so ans=9
 
so for a parent we need to have min 2 children
and then take min(max depth of 2 children) to ans
 
*/
...
void dfs(ll i, ll par){
   ...
   for(auto &son : adj[i]){
       dfs(son,i);
       ll maxdepth=depth[son]+1;
       if(maxdepth>maxi1){
        maxi2=maxi1;
        maxi1=maxdepth;
       } 
       else if(maxdepth>maxi2){
        maxi2=maxdepth;
       }
   }
   depth[i]=maxi1;
