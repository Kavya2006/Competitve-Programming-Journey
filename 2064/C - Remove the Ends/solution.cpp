...
 
// ------------------------- Main Logic ------------------------- //
// so if positive number is at starting
// we can delete that obv and take that 
// and if negative at end we can delete that too
// then we appear at dead end , if negative at start and positve at end
// so we can store like max of this or skip this and move forward type
// can i make like dp state , if like we decided to stop here
// then just store max till now somethingggggg
 
 
void Solve() {
    ll n; 
...
// nah man this approach is getting wrong
/*
-10 -3 -17 1 19 20
in this if we start from last negative and take all we get 30
and if we start from first positive and take all we get 40
so this can be the approach
 
like if 3 1 4 -1 -5 -9
// here if we start from first positive we get 23 , now if we start from last negaitve we take more like
 
// but what if its like 5 -10 15 -20 10 -4
 
// so like maintaing like all pos and neg
like pos prefix sum is 5 5 20 0 30 0
negative suff sum is  34 34 24 24 4 4
// so we should break at point where all preff[i]+suff[i] is max which is 
20+24=44
so we should take 5 , 15 and from negative side take -4 and then -20
// to get 44
 
 
*/
