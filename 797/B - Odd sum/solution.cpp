...
void Solve() {
    ...
    // so thing is if taking any negative element makes the sum odd then its good
    // otherwise its trash
    // if a1,a2,a3,a4,a5....an , we add all pos number to seq
    // then we subtract whicever is lower , abs value of negative or abs value of positve which is odd , like two pointer thing
    if(sum%2==1){
        cout<<sum<<endl;
        return;
    }
