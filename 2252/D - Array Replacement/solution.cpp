...
// ------------------------- Main Logic ------------------------- //
/*
so here also same logic as diff array of magic stones i just did
but thing is to do this operation parity should be same of a[i-1] and a[i+1]
a[i+1]-a[i-1]=(a[i+1]-a[i])+(a[i]-a[i-1])=d[i]+d[i-1]
so since a[i+1]-a[i-1] is even
d[i] and d[i-1] should have same parity
so for a contunious length if they have same parity we can sort the diff thing
and put it anywhere
*/
 
void Solve() {
    ...
    ll head=-1;
    while(tail<n-1){
        while(head+1<n-1 && (abs(d[head+1])%2==abs(d[tail])%2)){
            head++;
        }
        sort(d.begin()+tail,d.begin()+head+1);
        tail=head+1; 
    }
    for(ll i=1; i<n-1; i++){
        ans[i]=ans[i-1]+d[i-1];
