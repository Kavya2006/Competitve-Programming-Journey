...
void Solve() {
    ll n; 
    cin>>n;
    // so in how many ways we can make 17 digit palindrome
    // 1-9 choices , 2-10 choices.... 8-10 choices , 9-10 choices
    // so total approx 1e9 choices , so obv we cannot test all palindromes
    // oh my bad , like all 1 digit numbers are palindrome 
    // so suppose number is 300,  like 298 is divisble by 12 , so 298+2
    // but what if suppose number is like 22, closest less than this is 12
    // but 10 is not palindrome
    // so how can i prove for big numbers there wont be a pair or will be
    // like suppose a very big number =1200010 , here closest less than 12 is
    // oh so in this case i can like subtract 22 from it , 
    // coz 22 is palindrome and then 1200010-22 becomes divisble by 12
    // hell yeah , also like if its remainder with 12 is 11 , then 11 so no problem
    ll a,b;
    ll rem=(n%12);
    if(!rem){
        cout<<"0 "<<n<<endl;
