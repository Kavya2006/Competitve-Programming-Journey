...
 
void Solve() {
    // so number which cannot be formed will always be at start
    // then we proceed from this number and find others ig
    // also a number cannot occur again
    // as we are div by 3 and multiply by 2 so all number unique ig
    set<ll>s;
    ll n; cin>>n;
    vector<ll>a(n);
    for(ll i=0; i<n; i++){
    ...
    ll start;
    for(ll i=0; i<n ; i++){
        auto it=s.find(a[i]*3);
       if(a[i]%2==0){
          auto it2=s.find(a[i]/2);
