...
// ------------------------- Main Logic ------------------------- //
/*
so suppose n=3, 3*3+3+1
so basically like n*(n+1)+1=13
so now if n=2 suppose
so ans=1+n*(n+1)=1+2*3=7
its like 3-1-2
and 2,3 each connected to 2 more suppose 4,5 for 2 and 6,7 for 3
then each of 4,5,6,7 could be connceted to other as well
so +8=15 thats how we got the answer
so for 2 ans will be like 1+2+2*2+2*2*2+2*2*2*2
and so on so it will be 1+2+4+8.. so on , 1*(2^x-1)/2-1 = 2^x-1 , where x>=3
so for n=3 , we can say its 1+3+3*3+3*3*3 and so on
so ans is basically (3^x-1)/2 , where x starts from 3
so like 13 and go on
so like for 2 , 2^10=1e3 , so approx 2^60 we could cover till all 1e18 thing
for like 10 we go so for 10 its (10^x-1)/9
when we arrive at suppose , 1e6 , so (1e6^x-1)/(1e6-1)
so here we need to check only one value as 1e6^4 will cros1e24 thing
 
 
 
*/
...
void pre(){
  ...
  for(ll i=2; i<=1e6; i++){
     __int128 sum=1+i+i*i;
     __int128 curr=i*i;
     while(sum<=1000000000000000000LL){
        s.insert(ll(sum));
        curr=curr*i;
        sum=sum+curr;
     }
