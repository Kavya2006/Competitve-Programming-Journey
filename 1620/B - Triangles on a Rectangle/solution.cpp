...
 
    ...
    ll w,h; cin>>w>>h;
    ll k1 ; cin>>k1;
    vector<ll>horiontaly0(k1);
    for(int i=0 ; i<k1 ; i++){
        cin>>horiontaly0[i];
    }
    ll k2 ; cin>>k2;
    vector<ll>horiontalyh(k2);
    for(int i=0 ; i<k2 ; i++){
        cin>>horiontalyh[i];
    }
 
    ll k3 ; cin>>k3;
    vector<ll>verticalx0(k3);
    for(int i=0 ; i<k3 ; i++){
        cin>>verticalx0[i];
    }
   ll k4 ; cin>>k4;
    vector<ll>verticalxw(k4);
    for(int i=0 ; i<k4 ; i++){
        cin>>verticalxw[i];
    }
    cout<<max({abs(horiontaly0[0]-horiontaly0[k1-1])*h ,abs(horiontalyh[0]-horiontalyh[k2-1])*h  , abs(verticalx0[0]-verticalx0[k3-1])*w , abs(verticalxw[0]-verticalxw[k4-1])*w})<<endl;
 
 
 
 
