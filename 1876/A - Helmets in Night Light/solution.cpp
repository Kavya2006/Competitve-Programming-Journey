...
 
 ...
 for(int i=0 ; i<n ; i++){
   ...
   if(Pairs[i].first<nub){
    nub=nub-Pairs[i].first;
    count=count+Pairs[i].second*Pairs[i].first;
   }
   else if(Pairs[i].first>=nub){
    count=count+Pairs[i].second*nub;
    nub=nub-nub;
