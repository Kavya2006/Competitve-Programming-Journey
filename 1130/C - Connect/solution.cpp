...
while(!dq.empty()){
  ...
  vis[curr.F][curr.S]=1;
   firstt.push_back(curr);
  ll dx[]={1,0,-1,0};
  ll dy[]={0,1,0,-1};
  ll i=curr.F;
  ll j=curr.S;
  for(ll d=0 ; d<4 ; d++){
    if(possible(i+dx[d],j+dy[d]) && matrix[i+dx[d]][j+dy[d]]=='0'){
        dq.push_back({i+dx[d],j+dy[d]});
    }
  }
 
}
...
void Solve(){
cin>>n;
// i mean like n is upto 50 so i might just bruteforce it only
...
vector<pair<ll,ll>>firstt=dfs(x1,y1);
vector<pair<ll,ll>>secondd=dfs(x2,y2);
ll mini=INF;
// if they in same compo auto zero , no sep case ig
for(ll i=0 ; i<firstt.size(); i++){
    for(ll j=0 ; j<secondd.size(); j++){
        mini=min(mini,(firstt[i].F-secondd[j].F)*(firstt[i].F-secondd[j].F) + (firstt[i].S-secondd[j].S)*(firstt[i].S-secondd[j].S));
    }
}
cout<<mini<<endl;
 
