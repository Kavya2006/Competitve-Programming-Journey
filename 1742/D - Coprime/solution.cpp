...
// ------------------------- Main Logic ------------------------- //
/*
so i mark all numbers by their last occurence
// then for all 1000 numbers loop around
// so t.c=o(1000*1000*10) at worst case=10^7 which should pass ig
*/
 
void Solve() {
    ll n; 
    ...
    for(ll i=1; i<=1000; i++){
        for(ll j=1; j<=1000; j++){
            if(gcd(i,j)==1 && pos[i]!=-1 && pos[j]!=-1){
                ans=max(pos[i]+pos[j]+2,ans);
            }
        }
    }
    cout<<ans<<endl;
