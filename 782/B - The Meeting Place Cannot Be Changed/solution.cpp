...
 
bool possible(vector<ll>x,vector<ll>v,ll n,ld mid){
    ld lower=x[0]-v[0]*mid;
    ld higher=x[0]+v[0]*mid;
    for(ll i=1 ; i<n ; i++){
        if(lower>higher){
            return 0;
        }
        lower=max(lower,x[i]-v[i]*mid);
        higher=min(higher,x[i]+v[i]*mid);
    }
    if(lower>higher){
            return 0;
        }
...
ld low=0; ld high=1e9; // taking 1e9 as in 1*1e9 everyone can reach anywhere
ld ans=-1;
while(low<=high){
    ld mid=(low+high)/2;
    if(possible(x,v,n,mid)){
        ans=mid;
