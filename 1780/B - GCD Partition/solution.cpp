...
vector<ll>prefixleft(n);
prefixleft[0]=a[0];
for(int i=1 ; i<n ; i++){
 prefixleft[i]=a[i]+prefixleft[i-1];
}
vector<ll>prefixright(n);
prefixright[n-1]=a[n-1];
for(int i=n-2 ; i>=0 ; i--){
    prefixright[i]=a[i]+prefixright[i+1];
}
ll maxi=0;
for(int i=0 ; i<n-1 ; i++){
maxi=max(gcd(prefixleft[i],prefixright[i+1]),maxi);
}
cout<<maxi<<endl;
 
 
