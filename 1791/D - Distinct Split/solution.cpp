...
string s; cin>>s;
vector<ll>left(26,0);
vector<ll>leftfreq(n,0);
vector<ll>leftpre(n,0);
for(int i=0 ; i<n ; i++){
  if(left[s[i]-'a']==0){
    leftfreq[i]++;
    left[s[i]-'a']++;
...
leftpre[0]=1;
for(int i=1 ; i<n ; i++){
    leftpre[i]=leftfreq[i]+leftpre[i-1];
}
vector<ll>right(26,0);
vector<ll>rightfreq(n,0);
vector<ll>rightpre(n,0);
 
for(int i=n-1; i>=0 ; i--){
  if(right[s[i]-'a']==0){
    rightfreq[i]++;
...
rightpre[n-1]=1;
for(int i=n-2 ; i>=0; i--){
    rightpre[i]=rightfreq[i]+rightpre[i+1];
}
ll maxi=0;
for(int i=0 ; i<n-1 ; i++){
 maxi=max(leftpre[i]+rightpre[i+1],maxi);
}
cout<<maxi<<endl;
 
 
