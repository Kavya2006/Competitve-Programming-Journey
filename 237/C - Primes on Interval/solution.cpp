// from today, i mark my serious grind towards icpc
// lets go , i am all in , daily 10 cf problems more than my rating
// so i am 1230 so minimum 1300 rated 10 questions
// i may do some qs like lower than 1300 but it wont count towards my goal
 
 
 
 
...
vector<ll>spf(1e6+5);
 
bool possible(ll l , vector<ll>&pre){
 for(ll i=a; i<=b-l+1; i++){
    if(pre[i+l-1]-pre[i-1]<k){
        return 0;
    }
 }
 return 1;
...
// now ig i can mantain prefix sum of prime number
vector<ll>pre(b+2,0);
for(ll i=a; i<=b ;i++){
    if(spf[i]==i){
        pre[i]=pre[i-1]+1;
