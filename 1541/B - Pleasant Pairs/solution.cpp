...
/*
3 1 5 9 2
in this , so a[i].a[j]=i+j , a[i],a[j]<=2*n , i<j
so suppose i iterate from left
here i get 3 but no past so i take 3,1 myself
then i go to 1 , here i get 1,2 , now also 1*3=1+2 so its answer
but how do i do it fast as my curr approach would give me o(n*n) thing
 
so like i+j %a[i]==0 
so i+j should be multiple of a[i]
but what if all a[i] are 1 then we would be in o(n*n) ig 
lol finally got it all are distinct so 
t.c in worst=o(n*(1+1/2+1/3+1/4))=o(nlogn) lol
 
 
*/
 
void Solve() {
    ...
    ll ans=0;
    for(ll i=1; i<=n; i++){
        // so we need j such that j>i and j+i is multiple of a[i]
        // i+j= k*a[i] , j=k(a[i])-i;
        // j>i , so k*a[i]-i>i
        // so k>2*i/a[i]
        // so k=2*i/a[i]+1;
        // hmm i got wa now i got it ig
        // so j
        ll m=2*i/(a[i])+1;
        ll j=m*a[i]-i;
