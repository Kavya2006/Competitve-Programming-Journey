...
void dfs(ll n, ll m,ll i,vector<vector<ll>>&adj,vector<ll>&vis,ll &temp, vector<vector<ll>>&Matrix,ll j){
    vis[i]=j;
    temp+=Matrix[i/m][i%m];
    for(auto &neighbour : adj[i]){
        if(vis[neighbour]==0){
            dfs(n,m,neighbour,adj,vis,temp,Matrix,j);
        }
    }
}
 
...
for(ll i=0 ; i<n ; i++){
    for(ll j=0 ; j<m ; j++){
        ...
        for(ll d=0 ; d<4 ; d++){
            ll nx=i+dx[d];
            ll ny=j+dy[d];
            if(possible(nx,ny,n,m) && Matrix[i][j]!=0){
...
for(ll i=0 ; i<n*m ; i++ ){
    if(vis[i]==0){
      temp=0;
      j++;
      dfs(n,m,i,adj,vis,temp,Matrix,j);
