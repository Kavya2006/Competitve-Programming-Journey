...
 
// ------------------------- Main Logic ------------------------- //
// its easy just sum of all pairwise/total pairs
// will do it tomorrow morning as very sleepy rn , gn cf
// gm i am back brotherrr
// so (summation a)^2=(summation (a^2))+2*(summation ab)
// so summation ab=x-y/2
 
void Solve() {
    ll n; 
    cin>>n;
    ...
    sum=(sum*sum)%MOD;
    //  and nc2=n*(n-1)/2;
    ll ans=((sum-sumsq)%MOD+MOD)%MOD;
   
    ans=(ans*power((n*(n-1))%MOD,MOD-2,MOD))%MOD;
