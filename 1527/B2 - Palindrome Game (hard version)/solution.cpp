...
/*
 
now string is not necessarily a palindrome
so suppose it is not palindrome 
then alice can just leave and force bob to move
but suppose bob's move can make it a palindrome so instead alice make that move
and make it palindrome then bob moves then alice moves then bob and so on..
then at one point alice skips it and then bob moves ,so alice win
but suppose alice move make palindrome then bob move and now if game finishes only
its draw righttt.... ig
like suppose its 100 , if alice doesnt move , bob make palindrome then draw
so i have to take in account if only 2 moves like and one makes it pali then its draw ig
so suppose 1110110 , so suppose like this , if alice make palindrome bob move draw
and if alice skips then bob makes it and alice has to move
so i have to take this in account if 0s are 2 and taking any forces it to a pali
then answer is draw , otherwise allice
 
if it is palindrome in start only , then condition like last time
if one zero , bob 
if odd zero , alice
if even zero bob
 
