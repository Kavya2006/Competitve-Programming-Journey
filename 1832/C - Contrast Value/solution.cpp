...
void Solve() {
    ...
    // now b is a subsequence of a
    // so b should contain some elements of a
    // if cont==0 suppose then ans is obviously 1 any element
    if(!cont){
        cout<<1<<endl;
        return;
    }
    /*
    suppose a is 1 3 3 3 7
    so cont is 2+4=6 
    now at worst case i take all elements of a
    but can i do it better
    yes i can if only take 1 and 7 i get 6
    so am thinking if sequence is increasing or decreasing
    we ignore like values between them till it happens
    let me try to like apply it
 
    */
    ll ok=a[0];
    ll inc=0;
