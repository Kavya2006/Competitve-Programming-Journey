...
 
// ------------------------- Main Logic ------------------------- //
/* so see for a thing we calculate its gcd
so ant eaten = r-l+1-(no of ants with val=gcd)
lemme just use my template as i am very tired ig
 
*/
vector<ll>a;
struct Node{
...
Node Merge(Node left , Node right){
    Node res;
    res.gcdcount=0;
    res.gcdval=gcd(left.gcdval , right.gcdval);
    if(left.gcdval==res.gcdval){
       res.gcdcount+=left.gcdcount;
    }
    if(right.gcdval==res.gcdval){
