...
/*
n&(n+1)&(n+2)... = x
so we have to go on to match the bits basically
suppose n is 15 , x=10 
n+1=16 , and their and will get to 0 so here also -1 will be the answer 
so from n and x we have to compare bits
we have to go on till we get bits that matches bits of x
and if we go below x ans is -1
 
suppose n is something 1010101010 , and x is 000001010 
something like this
first of all their bits should match
like n&x==x otherwise ans is no
but here in this example it does
now we have to get to first number where other gets zero
like last number will be like not possible in this scene
coz i have to get 0 at start so like but others and would make number less than x
 
so msb of both the numbers should be same , only exception is zero ig
where we can just take next power of 2 
 
so suppose msb is same 
like 20,16
16=10000
20=10100 , so answer will be where they matches like where this second msb
gets zero
so 11000=24
*/
 
/*
back after 10 hrs so now try again
if(x>n){
    then not possible obv
}
also their msb should be same otherwise and would become zero like
so suppose n=20 = 10100 , x=16
                x=10000
so basically we would need to increase n so that its last non common increases
like i have to make it 11000 , =24 as ans
 
n=21, 10101 , x=16 , 10000 , so here also we have to increase till we get
11000 so and with all gives 16
now let n=25 suppose 11001 , x=17 , 10001
here its at second digit and it can't be increased so not possible ans here
 
n=29 , x=24
so we have 11101 , x=11000
so now first 2 matches it fine
but next 1 which is there not match but there in n
but left to it is 1 so here also not possible
n=11011 , x=11000 , here it would be possible
we could make it till 11100 , so yeah now just implement it
 
also x should be present in n , if not then ans is just not possible it would give 0
 
*/
 
void Solve() {
...
";
   ...
   for(ll i=st-1; i>=0; i--){
        if(nn[i]==1 && xx[i]!=1){
            ...
            ll sum=power_ll(2,i+1);
            for(ll j=i+2 ; j<=st; j++){
               sum+=power_ll(2,j)*nn[j];
            }
            cout<<sum<<endl;
            return;
        }
