...
3 10 15 8
2 12 11
now suppose we see , in 3 10 15 8
for 8 , 9 is needed , so check if 8 for prev can be done
but it can't be done so we need 16 for it and end can say would have 18
then will 15 work for 10 , yes it will , then 14 work for 3 
so for this segment start=14,end=18
now in 2 12 11, for 11, need 12 , then for 12 needed 11, no so we set
curr=13,end=15
then for 2 we would have 12
 
so now we have all st,end -> 12,15 and 14,18
 
so now we sort by st points
12,15
14,18 
and now ending>start of next so  fine ans=12
but if it was not
like it was 12,15 and 17,20 something
then we have to add+2 to start thing so ans is 14 here
and end basically becomes max of all ends like suppose we have
15,20 a interval
and 10,30 
so we start from 10 and end is 30 only, oh wait no
we have to make end as like end of this +(end-st) of next
so end would become 35
*/
 
void Solve() {
    ll n; 
