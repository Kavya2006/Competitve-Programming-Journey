...
void Solve(){
/*
we have to maximuise sum and k*x <=n
and sum is k*(k+1)*x/2 ;
so we no k*x<=n ; so we have to max k+1 , by giving it max value
and x min=2;
so kmax=n/2;
 
but in n=3 ; if x=3 and k=1 , sum is 3
and x=2 and k=1 sum 2 as very small value of n
but beyond 3 it will work
*/
ll n; cin>>n;
if(n==3){
    cout<<3<<endl;
