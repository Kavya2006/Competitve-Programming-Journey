...
/*
so suppose i sort
suppose i have 1 2 3 3 4 5 6 7  11 14  
// now obv first element has to be max to be thrown out
so now i want to find the element whose sum like exists something
// so my first choice can be like optimal thing
so if chose x=21 suppose , and chose 14,7 
// then i have 1,2,3,3,4,5,6,11 and x=14
// now i can remove 11,3
we get 1,2,3,4,5,6 and x=11
// now i remove 5,6
// so i have 1,2,3,4 , x=6
now i remove 4,2 and get 1,3 and x=4
// now remove 1,3
so yeah it worked this way
but what if i chose 14,11 then not work as no 2 other elements has sum=14
// so like when 14 gets removed , i need 2 elements whose sum=14
 
so like suppose in first operaiton i have to take largest element only
// and then i can go and see which i want , and then x will become largest
// and then 
 
 
*/
...
";
    ...
    for(ll i=2*n-2; i>=0; i--){
         ...
         op.push_back({a[i],a[2*n-1]});
         mss.erase(mss.find(a[i]));
         mss.erase(mss.find(a[2*n-1]));
         for(ll j=0; j<n-1 ; j++){
            auto it1=mss.end();
            it1--;
            ll maxi=*it1;
            mss.erase(it1);
            auto it2=mss.find(x-maxi);
            if(it2==mss.end()){
                ok=0;
                break;
            }
