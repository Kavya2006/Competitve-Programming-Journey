...
// coz log2(0) is not defined so it will return -1
 
...
f[0]=mex(a[0])
in 5 1
now we want f[0]^f[1]^f[2]^f[3]^f[4]=1
so last f[n-1] will always be like n
so we want is remaining to be n^1
here n is 5 , so 5^1=4
so we can have direct 4 made up
like 3,2,1,0,4
so f[0]=0,f[1]=0,f[2]=0,f[3]=0,f[4]=4,f[5]=5
so 4^5=1
 
so thing is like if n is suppose 8 and k is 16
then k cant be made up coz we cant provide the starting bit thing
n is 15 too then also we cant make 16
so if msb of of k>msb of n then its impossible we say
 
so like thing is now if n is 4 and k is 3
n^k=7
now its impossible to reach from 0,1,2,3 coz we dont have anything
here also we have to print like zero
but if n was 5 and k is 3
we can have 5^3=6
now 4^6=2
now we need 2
_,_,_,3,4 so 1,3,0,2,4 so yeah this works but how do i form it
 
 
*/
 
