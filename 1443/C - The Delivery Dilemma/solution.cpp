...
// ------------------------- Main Logic ------------------------- //
/* so let me cook ig
so all a[i] will come parallely only so we dont care about that mostly
so our max answer is max of all a[i] 
now we have to see if we can minimise this
to minimise we consider that last a[i] she take it and so on
so easy only ig
 
 
*/
...
void Solve() {
    ...
    for(ll i=0; i<n ; i++){
      if(i!=n-1){
      maxi=min(maxi,max(c[i+1].F,curr+c[i].S));
      curr+=c[i].S;
      }
      else{
        maxi=min(maxi,curr+c[i].S);
      }
    }
    cout<<maxi<<endl;
}
