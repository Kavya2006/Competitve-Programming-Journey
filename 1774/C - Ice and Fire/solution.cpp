...
void Solve() {
    ...
    /*
     4
     001
     // so for x=2 , there will be x-1=1 battle
     // between player 1 and 2 , and since its 0 , so 1 will win
     // x=3 , 2 battles , 1,2,3 whatever order one will win
     // now x=4 , 3 battles, 1,2,3,4 , 
     // so 1 cannot winn even coz last battle will be played at 1
     // 1 can play with any of 2,3,4 in first 2 , and whiche ever left will win
      so like 2,3,4 so 3 people can win
 
      now 101
      x=2 , obv 1 , x=3 , 10 , so 10 count , so last battle 0 , so anyone can win
      // except 3 , so 2 is the answer
      // x=4 , 101 , last battle 1 so anyone can win except 1 , so 3
 
 
      // suppose we have 1001010101000 like this thing
      // now lets take last x case here
      // so last battle is 0 , like last person cannot win , second last can
      // but for second last to put here , last should survive something befroe
      // but there is no hope
      // so n-2 , no coz third last is also 0 so n-1 and n both will be out
      // so answer is basically for each testcase , like index number- something
