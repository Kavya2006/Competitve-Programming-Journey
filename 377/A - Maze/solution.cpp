...
void dfs(ll i, vector<vector<ll>>&adj,vector<ll>&vis,ll &count , ll total , ll k){
    vis[i]=1;
    count++;
    if(count>total-k){
        vis[i]=-2;
    ...
    for(auto &neighbour : adj[i]){
        if(vis[neighbour]==0){
            dfs(neighbour,adj,vis,count,total,k);
        }
    }
    ...
    for(ll j=0; j<m ; j++){
        if(matrix[i][j]=='.'){
            for(ll k=0 ; k<4 ; k++){
                ll nx=i+dx[k];
                ll ny=j+dy[k];
                if(valid(nx,ny,n,m) && matrix[nx][ny]=='.'){
...
for(ll i=0 ; i<n ; i++){
    for(ll j=0 ; j<m ; j++){
          ...
          if(matrix[i][j]=='.'){
            dfs(i*m+j,adj,vis,count,total,k);
            possible=false;
            break;
          }
    }
