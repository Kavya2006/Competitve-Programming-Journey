...
 
// ------------------------- Main Logic ------------------------- //
// so what a losing state is when oponent cannot make move
// this can be if all elements 0's or 1 element is not zero
// coz like rest element would be 0 , and we cannot chose 0 so xor wont be 0
 
// so we have to put our opponent in a losing state
// suppose i have elements 3 9 12 4
// so from these n elments i have to make n-1 elements zero or all elements zero
// for all elements zero , xor of all elements should be 0
// so i xor all these elements , like find sum xor
// then for each indice remove a[i] , which is like sum^a[i]
// now we need a number suppose k such that newsum^k==0
// k==newsum
// so if k<=a[i] then its valid
// but what if its equal then we set like b[i] to a[i] so b thing is same only
// so extra counting will happen , so if all are equal we take only one only
//
 
 
 
