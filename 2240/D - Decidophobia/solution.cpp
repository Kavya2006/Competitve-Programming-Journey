...
 
// ------------------------- Main Logic ------------------------- //
// so just go through each person , give him gift and calculate if total gain or loss
// 
 
void Solve() {
    ll n,d; 
    ...
    ll ans=0;
    for(ll i=n; i<2*n ; i++){
        ans+=max(0LL,2*d*b[i]-(prefix[i+d]-prefix[i])-(prefix[i-1]-prefix[i-d-1]));
    }
    cout<<ans<<endl;
}
 
