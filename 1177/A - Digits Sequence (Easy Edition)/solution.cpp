...
void Solve(){
ll k; cin>>k;
// we have 1digit 9 positions
//         2 digit 90 positions
//         3 digit 900 positions
//         4 digit 9000 positions and so on
vector<ll>position(13);
position[1]=power_ll(10,0)*9;
for(ll i=2 ; i<13 ; i++){
    position[i]=position[i-1]+(power_ll(10,i-1)*9)*i;
}
if(k<=position[1]){
    cout<<k<<endl;
    return;
...
for(ll i=2 ; i<13 ; i++){
    ...
    ll ans=power_ll(10,i-1)+(k/i);
    string answer="";
    answer+=to_string(ans);
    cout<<answer[k%i]<<endl;
    return;
}
}
 
