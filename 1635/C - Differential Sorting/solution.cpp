...
// ------------------------- Main Logic ------------------------- //
/*
so basically a[n-2]<=a[n-1] otherwise answer is always no
// if last element is negative , a[n-1]<0 , then also no if it isnt sorted
coz suppose only 1 change required , but as it is inc , so all before this also -ve
excpet this
so that change mein whatever next element we created -(some element)
// that some element will be -ve
// so changed element = some next element + delta so yeah we can't do this
and 
if not suppose all conditions getting
so we can just put all elements into a[n-2]-a[n-1] and get increasing thing
 
*/
 
void Solve() {
