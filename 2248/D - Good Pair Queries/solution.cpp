...
// ------------------------- Main Logic ------------------------- //
/*
so char c must be mode of both thing
so suppose 001010
and        101011 
so total diff that can be formed are 01 ,00,11,10
so for 0 thing we definetly chose 00 , 
and for 1 thing we chose 11
now we want to think where to put 01 and 10
suppose i have 10 , 00 and 5-> 11 
and 13 01 and 2 10
so diff between 01 and 10 is 13-2=11
so if 00+11 count>=11 then fine otherwise answer is NO
because then mode would not be possible
 
 
*/
 
void Solve() {
    ...
    while(q--){
        ll l,r; cin>>l>>r;
        ll tot=a[r][0]-a[l-1][0]+a[r][1]-a[l-1][1];
        ll nub=abs(a[r][2]-a[l-1][2]-(a[r][3]-a[l-1][3]));
        if(nub<=tot){
            cout<<"YES
";
        }
