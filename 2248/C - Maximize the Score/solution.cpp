...
ll rec(ll i){
    ...
    ll maxi=0;
    if(pos[a[i]][0]==i){ // am getting first occ so i can take it with its second occurence
      ll len=pos[a[i]][1]-pos[a[i]][0]+1;
        maxi=(rec(pos[a[i]][1]+1)+len*len);
    }
    maxi=max(maxi,rec(i+1)+1);
    return dp[i]=maxi;
}
 
void Solve() {
