...
void Solve(){
  ...
  ll mini=q[0];
  vector<ll>finall;
  finall.push_back(q[0]);
  for(ll i=1 ; i<m ; i++){
        if(q[i]<mini){
            finall.push_back(q[i]);
            mini=q[i];
        }
  }
sort(finall.begin(),finall.end());
vector<ll>presum(finall.size());
presum[0]=power_ll(2,finall[0]-1);
for(ll i=1 ; i<finall.size() ; i++){
    presum[i]=presum[i-1]+power_ll(2,finall[i]-1);
}
for(ll i=0 ; i<n ; i++){
    for(ll j=finall.size()-1 ; j>=0 ; j--){
        if(a[i]%power_ll(2,finall[j])==0){
            a[i]+=presum[j];
            break;
        }
    }
