...
/*ll powera(ll a, ll b, ll mod = MOD) {      // we can change value up there MOD 
    ll res = 1;
    while (b > 0) {
        if (b % 2 == 1)
            res = (res * a) % mod;
...
*/
ll power(ll a, ll b) {          // if we dont want to take the mod of result 
    ll res = 1;
    while (b > 0) {
        if (b & 1) res *= a;
        a *= a;
...
for(int i=0 ; i<n ; i++){
    ...
    ans=ans+power(2,white)+power(2,black)-2-white-black;
}
for(int j=0 ; j<m ; j++){
    ll white=0;
    ll black=0;
