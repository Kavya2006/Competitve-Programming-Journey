...
 
  Segtree[2*i].lazy+=x;
  Segtree[2*i].val+=x;
  Segtree[2*i].isLazy=1;
  Segtree[2*i+1].lazy+=x;
  Segtree[2*i+1].val+=x;
  Segtree[2*i+1].isLazy=1;
  Segtree[i].lazy=0;
  Segtree[i].isLazy=0;
}
Node Merge(Node left, Node right){
...
void Solve() {
    ...
    ll m; cin>>m;
    string dummy;
    getline(cin,dummy); //To Consume The Stray Newline
    while(m--){
        string line;
        getline(cin,line);
        ...
        if(ss>>v){
            ...
            if(l<=r){
            Node ok=Query(1,0,n-1,l,r);
            cout<<ok.val<<endl;
            }
            else{
                Node left=Query(1,0,n-1,l,n-1);
                Node right=Query(1,0,n-1,0,r);
                cout<<min(left.val,right.val)<<endl;
            }
        }
    }
}
