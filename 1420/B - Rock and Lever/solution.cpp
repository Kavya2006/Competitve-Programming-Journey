...
/*
a[i]&a[j]>=a[i]^a[j]
so if suppose like a[i] is 11101010101 something like this
and a[j] should also have bit at the last position where a[i] has
otherwise their xor would become greater than and thing
so i just go and find the highest pos their bit is stored and easy ig
 
*/
ll bits(ll i){
    ll ans=0;
